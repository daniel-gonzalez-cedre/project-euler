...
h   = np.max([np.prod([grid[i][j + k]       for k in range(0, 4)]) for i in range(0, m)     for j in range(0, n - 4)])
v   = np.max([np.prod([grid[i + k][j]       for k in range(0, 4)]) for i in range(0, m - 4) for j in range(0, n)])
ld  = np.max([np.prod([grid[i + k][j + k]   for k in range(0, 4)]) for i in range(0, m - 4) for j in range(0, n - 4)])
rd  = np.max([np.prod([grid[i + k][j - k]   for k in range(0, 4)]) for i in range(0, m - 4) for j in range(3, n)])
print(max(h, v, ld, rd))
